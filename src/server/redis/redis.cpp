...
bool Redis::connect() 
{
    ...
    // 订阅是阻塞的，所以要在单独的线程中，监听通道上的事件，有消息给业务层上报
    thread t([&](){
...
{
    // SUBSCRIBE命令本身会造成线程阻塞等待通道里发生消息，这里只做了订阅通道，不接受通道消息
    // 通道消息的接收专门在observer_channel_message函数中的独立线程中进行
    // 只负责发送命令，不阻塞接收redis server响应消息， 否则和notifyMsg线程抢占响应资料
    if (REDIS_ERR == redisAppendCommand(this->_subscribe_context, "SUBSCRIBE %d", channel)) 
    {
        cerr << "subscribe command failed! 1" << endl;
        return false;
    }
    ...
    while (!done)
    {
        ...
        {
            cerr << "subscribe command failed! 2" << endl;
            return false;
        }
    }
    // redisCommand : redisAppendCommand + redisBufferWrite + redisGetReply阻塞
    return true;
}
